...
class IntCode(object):
    ...
    def _get_instruction(self):
        self.instruction = self.state[self.anchor]
        
    def _get_opcode_and_params(self):
        self.opcode = str(self.instruction).zfill(4)[-2:]
        self.params = str(self.instruction).zfill(4)[:-2]
...
#         self.update_anchor()
    ...
    def compute(self, state=None, inputs=None, verbose=False):
        ...
        while 1:
            self._get_instruction()
            if verbose: print('Instruction: ', self.instruction)
            if self.instruction == 99:
                return np.nan
            self._get_opcode_and_params()
            if verbose: print('Opcode: ', self.opcode, '\nParams', self.params)
